...
function loadPlatformException(){
	$.post(pt.ctx+'/web/gateway/wizard/getPlatformExceptions.action',function(data){
		if(data.success == true){
			if(data.platformException.storageCount > 0){
				$('#storageCount').html(data.platformException.storageCount);
			...
			if(data.platformException.deviceExceptionCount > 0){
				$('#deviceExceptionCount').html(data.platformException.deviceExceptionCount);
			...
			if(data.platformException.deviceInfoExceptionCount > 0){
				$('#deviceInfoExceptionCount').html(data.platformException.deviceInfoExceptionCount);
			...
			if(data.platformException.nodeExceptionCount > 0){
				$('#serviceExceptionCount').html(data.platformException.nodeExceptionCount);
